...
async function boot(html) {
  ...
  // ★`js/tarot.js` 는 **별도 파일**이다. index.html 만 부팅하면 타로 경로가 통째로 안 보인다.
  w.eval(TAROT_SRC);
  await sleep(250);
  return w;
}
...
(async function main() {
  ...
  check('B-8', '★단일 출처(결정 99·113) — 산출식 `cwBirthKeys` 1개 · 적재 지점 5곳이 **명시 리터럴**이다', () => {
    ...
    const inHtml = (HTML.match(LIT) || []).length;      // 작명 사람 무료 · 닉네임 · `_buildNamingContext`
    const inTarot = (TAROT_SRC.match(LIT) || []).length;  // 타로 무료 · 타로 프리미엄
    if (inHtml !== 3 || inTarot !== 2)
      return { ok: false, detail: '★적재 리터럴 index.html ' + inHtml + '(기대 3) · js/tarot.js ' + inTarot + '(기대 2) — 파 ⓐ 적재 지점은 5곳이다' };
    ...
    return { ok: true, detail: '산출식 1개 · 적재 리터럴 5곳(index.html 3 + js/tarot.js 2) · 반입 블록 밖' };
  });
