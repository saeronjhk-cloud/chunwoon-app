...
NAMES = {
    ...
    # ── 1단계-v3: ★배정 규칙 v3 (P-59 · 2026-08-29)
    #   ★트리(P-53)가 찾은 축을 ★손규칙으로 옮긴 것입니다.
    #   ★★구조는 Tier-C 에서 배웠지만 ★값은 ★레지스트리를 거칩니다 —
    #      ⟹ ★G-7 이 그대로 적용됩니다 (트리는 우회했습니다).
    "V3_UMAX_DTC":       "u_max 가 이 값을 넘으면 DTC (손바닥을 가로질러 자쪽까지 뻗음)",
    "V3_UMAX_RLC":       "u_max 가 이 값 이하이면 RLC (엄지 쪽에 머묾)",
    "V3_VMAX_MID":       "v_max 가 이 값 이하이면 아래쪽 무리(FATE/NONE), 초과면 위쪽(RLC/PTC)",
    "V3_VMAX_LOW":       "v_max 가 이 값 이하이면 NONE (손목 근처 잡음)",

    # ── 1단계-v4: ★★배정 규칙 v4 (P-63 · 2026-08-31)
    #   ★★v3 와 ★출처가 다릅니다. ★v3 는 ★Tier-C(스튜디오 박스 라벨)에서,
    #      ★v4 는 ★★Tier-A(제이 손 23장 · 조각 정답 212개)에서 왔습니다.
    #   ★★그래도 ★1명 2손입니다 — ★freeze 전까지 ★verdict 에서 쓰이지 않습니다 (G-7).
    #   ★축이 v3 와 다릅니다: v3=u_max·v_max / ★v4=★cu·ang_v_deg (P-58 · P-60 실측)
    #      ⟹ ★우리 조각은 잘아서 u_max ≈ cu 가 되어 ★u_max 가 정보를 잃습니다.
    "V4_CU_SPLIT":       ("cu 가 이 값 이하이면 ★노쪽(엄지 쪽) 무리, 초과면 자쪽 무리. "
                          "★v4 의 1차 분기입니다"),
    "V4_ANG_RLC":        "노쪽에서 ang_v_deg 가 이 값 이하이면 RLC (엄지두덩을 감싸는 호)",
    "V4_VMAX_MID":       "노쪽·가로 조각 중 v_max 가 이 값 이하이면 NONE, 초과면 PTC",
    "V4_ANG_DTC":        "자쪽에서 ang_v_deg 가 이 값을 넘으면 DTC (손가락 쪽 가로선)",
    "V4_CV_FATE":        "자쪽·세로 조각 중 cv 가 이 값 이하이면 FATE, 초과면 NONE",

    # ── 1.5단계: 조각 병합 (P-24 · 2026-08-29)
    "MERGE_DIST":        ("두 조각의 끝점 거리 / wid — 이하이면 이을 후보. "
                          "★너무 키우면 ★서로 다른 선이 붙어 ★정확도가 떨어집니다(실측)"),
    "MERGE_ANG":         "두 조각의 주축 방향 차이 상한(도) · 이음선 방향도 같은 기준",
...
DEV_DEFAULTS = {
    ...
    # ★P-24 실측 최적 (C_train 47장 · 267조각 · F1 0.705 → 0.774)
    #   ★★표본이 작고 ★학습 도메인입니다 — ★우리 도메인 검증 전에는 ★기본 off 입니다
    "MERGE_DIST": 0.10,
    "MERGE_ANG": 55.0,
    # ★P-59 — ★트리(depth 3)의 분기점을 ★그대로 옮긴 출발점입니다.
    #   ★★Tier-C 에서 온 값이므로 ★verdict 에서는 ★freeze 전까지 쓰이지 않습니다.
    "V3_UMAX_DTC": 0.5008,
    "V3_UMAX_RLC": 0.2054,
    "V3_VMAX_MID": 0.6357,
    "V3_VMAX_LOW": 0.2376,
    # ★★P-63 (2026-08-31) — ★우리 도메인 트리(depth 3)의 분기점을 ★그대로 옮긴 값입니다.
    #   ★출처: ourtree.py · A-smoke 23장 · 제이 조각 정답 212개 (★Tier-A)
    #   ★★212개 ★전체로 적합한 값입니다 ⟹ ★이 값으로 212 를 채점하면 ★in-sample 이라 부풀려집니다.
    #      ★정직한 수치는 ★p63_rule.py 의 ★k-fold(폴드마다 재적합) 쪽입니다.
    "V4_CU_SPLIT": 0.0340,
    "V4_ANG_RLC": 28.0318,
    "V4_VMAX_MID": 0.7133,
    "V4_ANG_DTC": 47.7215,
